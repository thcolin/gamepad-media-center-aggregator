...
RemoteAdd::RemoteAdd(std::function<void()> onDone, int editIndex) : onDone(onDone), editIndex(editIndex) {
    ...
    ParsedRemote p;
    if (editIndex >= 0) p = parseRemote(remotes[editIndex]);
    else p.port = remoteSchemes[0].port;
    this->typeIndex = p.type;

    this->header->setTitle(editIndex >= 0 ? "main/remote/edit_server"_i18n : "main/remote/add_server"_i18n);
    ...
    this->cellUser->init(
        "main/remote/username"_i18n, p.user, [](std::string) {}, "main/remote/optional"_i18n, "", 64);
...
void RemoteAdd::submit() {
    ...
    brls::async([ASYNC_TOKEN, r]() {
        ...
        brls::sync([ASYNC_TOKEN, r, ok, error]() {
            ASYNC_RELEASE
            ...
            if (ok) {
                ...
                Dialog::cancelable(fmt::format("{}\n\n{}",
                                       fmt::format(fmt::runtime("main/remote/test_failed"_i18n), error),
                                       "main/remote/retry"_i18n),
                    [this]() { this->submit(); });
            }
        });
    });
