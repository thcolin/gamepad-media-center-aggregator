/*
    ...
    AppConfig live, preserving the single-active-server behavior. See MULTI_BACKEND.md §7.
*/
...
class PlexBackend : public media::Backend {
public:
    media::BackendType type() const override { return media::BackendType::Plex; }
    ...
    void getHomeHubs(
        int count, bool excludeContinueWatching, media::Then<media::Container<media::Hub>> then,
        media::OnError error) override;
    void getSectionHubs(
        const std::string& sectionId, int count, media::Then<media::Container<media::Hub>> then,
        media::OnError error) override;
    ...
    std::string imageUrlExternal(const std::string& absoluteUrl, int width, int height) const override;
    std::string downloadUrl(const std::string& partKey) const override;
    HTTP::Header authHeaders() const override;

    // ---- personal list: plex.tv account watchlist ----------------------------
    ...
    void matchInLibrary(const std::string& guid, media::Then<media::Item> then, media::OnError error) override;
